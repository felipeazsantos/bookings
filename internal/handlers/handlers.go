...
import (
	...
	"github.com/felipeazsantos/bookings/internal/repository"
	"github.com/felipeazsantos/bookings/internal/repository/dbrepo"
...
// ShowLogin handler for login user authenticate
...
// AdminNewReservations shows all new resevations in admin tool
...
// AdminReservationsCalendar displays the reservation calander
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		// that does not exists in our posted data, and if the restrictionId > 0, then it's a block we neeed to remove
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int)

		for name, value := range curMap {
			// ok will be false if the value is not in the map
			if val, ok := curMap[name]; ok {
				// only pay attendation to values > 0, and that are not in the form post
